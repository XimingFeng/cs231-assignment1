...
class KNearestNeighbor(object):
  ...
  def compute_distances_no_loops(self, X):
    ...
    X_train_square = np.reshape(X_train_square, (1, num_train))
    
    dists = X_square  + X_train_square - 2 * product    
    ...
    for i in xrange(num_test):
      ...
      for j in range(len(count)):
        if temp < count[j]:
            temp = count[j]
            most_common_label = j
      ## print("for test point", i, ", the most common label is ", most_common_label)
